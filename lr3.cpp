...
//
// "Algorithmic Manipulation of Third-Order Linear Recurrences, Stanley Rabinowitz"
//
// Note : Equation between 8 and 9 is the most important : it connects the vector to the regular sequence
// When p,q,r are 0,1,1 W[n] are the Perrin numbers.
//
// ------------------------------------------------------------------------
...
bool lnrcr3(int64_t p, int64_t q, int64_t r, mpz_t n)
{
	...
	do {
		...
		while (bit) {
			...
			// modular reduction once per loop since intermediate numbers are O(n^2)
			mpz_mod(xm, qx, n);
			mpz_mod(ym, qy, n);
			mpz_mod(zm, qz, n);
		}
		...
		// compute S[n]
		// Equation (8.5 "Another sequence of interest" : ZZ1 is W[n])
		mpz_mul_si(z1, xm, S2);
		mpz_mul_si(t, ym, S1);
		mpz_add(z1, z1, t);
		mpz_mul_si(t, zm, S0);
		...
		// compute S[1]
		// Equation (8.5 "Another sequence of interest" : ZZ2 is W[1])
		mpz_set_si(z2, S1);
		mpz_mod(zz2, z2, n);
		...
		// Equation (4.5 "small values of n" : ZZ2 is W[-1])
		mpz_set_si(xq1, 1);
		mpz_set_si(yq1, p);
		mpz_mod(yq1, yq1, n);
		mpz_sub(yq1, n, yq1);
...
// addition rx,ry,rz = ax,ay,az + bx,by,bz,  (tx,ty,tz are temp variables) 
// Equation(16)
...
#define add(rx, ry, rz, ax, ay, az, bx, by, bz, tx, ty, tz) \
	do { \
		...
		for (unsigned i = 2; i < 4; i++) {
			...
			// Equation (8.5 "Another sequence of interest" : ZZ1 is W[-i*n])
			...
			// Equation (8.5 "Another sequence of interest" : ZZ2 is W[-i])
